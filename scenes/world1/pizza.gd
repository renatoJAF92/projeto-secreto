...
@export var flight_height: float = -70.0  # flies this many px above spawn y

@export_group("Attack")
@export var detection_range_x: float = 180.0
@export var detection_range_y: float = 200.0
...
@onready var hurt_zone: Area2D = $BodyHitbox
...
func _do_patrol(delta: float) -> void:
	...
	position.y = _origin.y + flight_height + sin(Time.get_ticks_msec() * 0.005) * 5.0
...
func die() -> void:
	...
	sprite.play("death")
	get_tree().create_timer(0.4, true).timeout.connect(queue_free, CONNECT_ONE_SHOT)
