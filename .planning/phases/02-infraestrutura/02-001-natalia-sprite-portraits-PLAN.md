...
<task type="auto">
  ...
  <action>
    Criar `scripts/generate_sprites.py` seguindo as convenções de `serve.py`: shebang `#!/usr/bin/env python3`, docstring com bloco `Uso:` (`python3 scripts/generate_sprites.py` a partir da raiz do projeto), `from PIL import Image`, constantes `PHOTOS_DIR = "Photos"` e `OUTPUT_DIR = "assets/sprites"` (atenção à capitalização `Photos` — confirmada no filesystem), guard `if __name__ == '__main__':`.
  ...
  <acceptance_criteria>
    ...
    - Comando `python3 scripts/generate_sprites.py` na raiz sai com código 0
